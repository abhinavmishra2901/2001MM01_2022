# Project 3 CS384 -     Web Based Interface for PSAT using Streamlit
...
st.set_page_config(page_title="Web Based Interface for PSAT")
...
def proj_psat_gui():
	try:
		...
		def add_front_name(name,x):
			...
			return name
		st.write('-'*25)
		tch=st.selectbox('Choose Filtering Method:', ['1. C','2. S','3. A','4. C & S','5. C & A','6. S & A','7. C & S & A','8. all combine'])
		if int(tch[0]) == 1:
			corr = int(st.text_input('Enter thresold value C:', value=70))
		elif int(tch[0]) == 2:
			SNR = int(st.text_input('Enter thresold value S:', value =15))
		elif int(tch[0]) == 3:
			Lambda = st.number_input('Enter Lambda value for A:', value=1.5)
			k = st.number_input('Enter k value for A:', value=1.5)
			st.write(Lambda,k)
		elif int(tch[0]) == 4:
			corr = int(st.text_input('Enter thresold value C:', value=70))
			SNR = int(st.text_input('Enter thresold value S:', value=15))
			...
			Lambda = st.number_input('Enter Lambda value for A:', value=1.5)
			k = st.number_input('Enter k value for A:', value=1.5)
		elif int(tch[0]) == 7 or int(tch[0])==8:
			corr = int(st.text_input('Enter thresold value C:', value=70))
			SNR = int(st.text_input('Enter thresold value S:', value=15))
			Lambda = st.number_input('Enter Lambda value for A:', value=1.5)
			k = st.number_input('Enter k value for A:', value=1.5)
		...
		sch=st.selectbox('Choose Replacement Method',['1. previous point','2. 2*last-2nd_last','3. overall_mean',
...
# if ver == "3.8.10":
# 	st.write("Correct Version Installed")
# else:
# 	st.write("Please install 3.8.10. Instruction are present in the GitHub Repo/Webmail. Url: https://pastebin.com/nvibxmjw")
